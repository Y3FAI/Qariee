...
export default function ReciterDetailScreen() {
  ...
  const { downloadSurah, deleteDownload, cancelDownload, isDownloaded: checkDownloaded, getProgress } = useDownload();
  ...
  const handleDownloadSurah = async (surah: Surah) => {
    ...
    const isCurrentlyDownloading = !!(progress && (progress.status === 'downloading' || progress.status === 'queued'));
  ...
  const renderSurahItem = (surah: Surah) => {
    ...
    const isDownloaded = checkDownloaded(reciter.id, surah.number);
    const progress = getProgress(reciter.id, surah.number);
    const isDownloading = !!(progress && (progress.status === 'downloading' || progress.status === 'queued'));
    const isDeleting = !!(progress && (progress.status as any) === 'deleting');
        ...
        >
          {isDownloading || isDeleting ? (
            <View style={{ direction: 'ltr' }}>
              <CircularProgress
                size={24}
                progress={isDeleting ? 0 : (progress?.progress || 0)}
                strokeWidth={2.5}
            ...
            <Ionicons
              ...
              size={24}
              color={isDownloaded ? '#efefd5' : isOffline ? '#4A4A4A' : '#B3B3B3'}
            />
          )}
        </TouchableOpacity>
      </TouchableOpacity>
        ...
        <View style={[styles.actionButtons, rtl && styles.actionButtonsRTL]}>
          ...
          <TouchableOpacity
            style={[
              ...
              (isBatchDownloading || isOffline) && styles.downloadAllButtonDisabled
            ]}
          ...
          >
            <Text
              style={[styles.downloadAllText, { fontFamily: getFontFamily(arabic, 'semibold') }]}
              numberOfLines={1}
              ellipsizeMode="tail"
            >
              {isBatchDownloading
                ? `${allDownloaded ? t('removing') : t('downloading')}... ${downloadedCount}/${totalCount}`
                : allDownloaded
                ? t('remove_all')
                : t('download_all')}
            </Text>
