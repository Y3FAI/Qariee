...
import MediaControl, {
    Command,
    PlaybackState as MediaPlaybackState,
...
class AudioService {
            ...
            if (this.currentTrack && this.isOffline) {
                try {
                    ...
                    if (!localPath) {
                        ...
                        console.log(
                            "[AudioService] Going offline, current track not downloaded - stopping playback",
                        )
                        await this.pause()
                    ...
                    console.error(
                        "[AudioService] Error checking current track download status:",
                        error,
                    )
                }
            }
    ...
    private async rebuildQueue() {
        ...
        if (this.isOffline && remainingTracks.length > 0) {
            ...
            remainingTracks = downloadedTracks.filter(
                (t): t is Track => t !== null,
            )
        }
    ...
    private async updateMediaControlPositionFromStatus(status: AudioStatus) {
        ...
        const now = Date.now()
        if (
            now - this.lastMetadataUpdateTime <
            AudioService.METADATA_UPDATE_THROTTLE_MS
        ) {
            return
        }
    ...
    private shuffleWithHistory(tracks: Track[]): Track[] {
        ...
        const availableTracks = tracks.filter(
            (track) =>
                !this.shuffleHistory.some(
                    (historyTrack) =>
                        this.getTrackId(historyTrack) ===
                        this.getTrackId(track),
                ),
        )

        // If no tracks available after filtering, clear history and use all tracks
        const tracksToShuffle =
            availableTracks.length > 0 ? availableTracks : [...tracks]
    ...
    async resume() {
        if (!this.player) {
            console.warn(
                "[AudioService] Cannot resume - player not initialized",
            )
            return
        }
    ...
    setSleepTimer(minutes: number, onComplete?: () => void) {
        ...
        this.sleepTimerTimeout = BackgroundTimer.setTimeout(() => {
            console.log(
                "[AudioService] Sleep timer complete - pausing playback",
            )
            this.pause()
            this.resetVolume()
    ...
    async playNext() {
        try {
                ...
                try {
                    ...
                    if (this.currentTrack) {
                        await MediaControl.updateMetadata({
                            title: this.currentTrack.surahName,
                            artist: this.currentTrack.reciterName,
                            artwork: getReciterPhotoUrl(
                                this.currentTrack.reciterId,
                            )
                                ? {
                                      uri: getReciterPhotoUrl(
                                          this.currentTrack.reciterId,
                                      ),
                                  }
                                : undefined,
                    ...
                    try {
                        ...
                        const audioSource =
                            localPath || this.currentTrack.audioUrl
                        this.player?.replace(audioSource as AudioSource)
                        this.player?.play()
            ...
            while (
                ...
                if (this.isOffline) {
                    const hasFile = await downloadService.isDownloaded(
                        nextTrack.reciterId,
                        nextTrack.surahNumber,
                    )
                    if (!hasFile) {
                ...
                try {
                    ...
                    console.error(
                        `[AudioService] ❌ Failed to play ${nextTrack.surahName}, skipping...`,
                        error,
                    )
    ...
    async playPrevious() {
        ...
        try {
            ...
            const previousTrack =
                this.playedTracksOrder[this.playedTracksOrder.length - 2]
            if (!previousTrack) {
                return
            }
